...
rm_panel_files() {
  output "Removing butterfly panel files..."
  ...
  success "Removed butterfly panel files. Goodbye :("
}
...
rm_wings_files() {
  output "Removing the butterfly's wing..."
  ...
  success "Removed the butterfly's wings."
}
...
rm_database() {
  ...
  if [[ "$valid_db" == *"panel"* ]]; then
    echo -n "* Database called panel has been detected. Is it the butterfly database? (y/N): "
    read -r is_panel
    if [[ "$is_panel" =~ [Yy] ]]; then
      DATABASE=panel
    else
  ...
  while [ -z "$DATABASE" ] || [[ $valid_db != *"$database_input"* ]]; do
    echo -n "* Choose the butterfly panel database (to skip don't input anything): "
  ...
  while [ -z "$DB_USER" ] || [[ $valid_users != *"$user_input"* ]]; do
    echo -n "* Choose the butterfly panel user (to skip don't input anything): "
    read -r user_input
