...
# download URLs
export PANEL_DL_URL="https://github.com/pterodactyl/panel/releases/latest/download/panel.tar.gz" # Replace later
...
export GITHUB_BASE_URL=${GITHUB_BASE_URL:-"https://raw.githubusercontent.com/mrdiploben/ButterflyInstaller"}
...
welcome() {
  ...
  print_brake 70
  output "Butterfly panel installation script @ $SCRIPT_RELEASE"
  output ""
  output "Copyright (C) 2024 - 2024, Ben Matthews, <Ben@craftcloud.host>"
  output "https://github.com/mrdiploben/ButterflyInstaller/"
  output ""
  output "This script is fully supported with butterfly panel. And works to date."
  output ""
  output "Running $OS version $OS_VER."
  if [ "$1" == "panel" ]; then
    output "Latest butterfly/panel is $PTERODACTYL_PANEL_VERSION"
  elif [ "$1" == "wings" ]; then
...
get_latest_versions() {
  ...
  PTERODACTYL_PANEL_VERSION=$(get_latest_release "mrdiploben/butterfly-Panel")
